...
route.use(
  cors({
    origin: ["https://todoapp-puce-alpha.vercel.app","https://playshaban.github.io"],
...
route.post("/addnote", (req, res) => {
  ...
  const currtime = temptime.toLocaleString("en-US", {
...
route.get("/getdata/:day/:month/:year/:userId", async (req, res) => {
  ...
  try {
    const result = await Note.find({ date: currdate, userId }).sort({
      checked: 1,
    });
...
route.post("/contact", async(req, res) => {
  try {
    ...
    console.log("Internal Server Error In Contact");
  }
});
